...
//                            print("Driver Location: \(driverLoc), (\(self.distanceString(distance)))")
    ...
    @IBAction func callPressed(sender: AnyObject) {
        if requestActive {
            ...
            callButton.setTitle("Call a Grüber", forState: .Normal)
            requestActive = false
        }
        else {
            ...
            request.saveInBackgroundWithBlock {
                ...
                if success {
                    self.callButton.setTitle("Cancel Grüber", forState: .Normal)
                    self.requestActive = true
                } else {
                    let errorMessage = error?.localizedDescription

                    let alert = UIAlertController(title: "Could not call Grüber", message: errorMessage! + "\nPlease try again", preferredStyle: .Alert)
